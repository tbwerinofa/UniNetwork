...
{
    public class VenueBL : IEntityViewLogic<VenueViewModel>
    {
        protected readonly SqlServerApplicationDbContext _context;


        #region Constructors
        public VenueBL(SqlServerApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public ResultSetPage<VenueViewModel> GetEntityListBySearchParams(
          GridLoadParam paramList)
        {

            var propertyInfo = typeof(VenueViewModel).GetProperty(paramList.SortField);

            var resultSet = _context.Venue
                 .IgnoreQueryFilters()
        ...
        public async Task<VenueViewModel> GetEntityById(
              int? Id, AuthorizationModel model = null)
        {
            ...
            if (Id > 0)
            {
                var entity = await _context.Venue.IgnoreQueryFilters()
        ...
        public async Task<SaveResult> SaveEntity(VenueViewModel viewModel)
        {
            SaveResult saveResult = new SaveResult();
                ...
                try
                {
                    if (viewModel.Id != 0)
                    {
                        if (_context.Venue.IgnoreQueryFilters().Any(a => a.Id == viewModel.Id))
                        {
                            entity = await _context.Venue.IgnoreQueryFilters().FirstOrDefaultAsync(a => a.Id == viewModel.Id);
                        }
                        entity = viewModel.ToEntity(entity);
                        _context.Venue.Update(entity);
                    }
                    else
                    {
                        entity = viewModel.ToEntity(entity);
                        _context.Venue.Add(entity);
                    }
        ...
        public async Task<SaveResult> DeleteEntity(int Id)
        {
            ...
            try
            {
                var entity = await _context.Venue.IgnoreQueryFilters().FirstOrDefaultAsync(a => a.Id == Id);
